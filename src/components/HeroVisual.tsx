  ...
  return (
    <div className="hero-visual" aria-hidden="true">
      ...
      <div className="hero-visual-mobile">
        <div className="mobile-doc-row">
          {documents.map((doc) => (
            <span key={doc}>{doc}</span>
          ))}
        </div>
        <div className="mobile-flow">
          {["Classify", "Extract", "Validate", "Review"].map((step, index) => (
            <div
              className={index === 3 ? "mobile-flow-step review" : "mobile-flow-step"}
              key={step}
            >
        ...
        <div className="mobile-gate">Human Review Gate</div>
      </div>
    </div>
  );
}
